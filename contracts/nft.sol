...
contract awnProxy is ERC721, Ownable {
    ...
    mapping(uint256 => mapping(string => bidEvidence[])) public  deliveryOnChain;
    mapping(uint256 => escrowInfo[]) public escrowOnChain;
    mapping(string => string) public kvMap;
    ...
    function mintArtWorksToken(artWork memory aw, artWorkImage[] memory awImages, uint256 tokenId, string memory tokenURI) public onlyOwner {
        bytes memory rData = abi.encodeWithSignature("mintArtWorksToken(((string,string,string,string,string,string,string,string,string,string,string,string,string),bool),(string,string)[],uint256,string)", aw, awImages, tokenId, tokenURI);
        execute(rData);
    }
    
    function mintArtWorksToken2Owner(artWork memory aw, artWorkImage[] memory awImages, uint256 tokenId, string memory tokenURI, address owner) public onlyOwner {
        bytes memory rData = abi.encodeWithSignature("mintArtWorksToken2Owner(((string,string,string,string,string,string,string,string,string,string,string,string,string),bool),(string,string)[],uint256,string,address)", aw, awImages, tokenId, tokenURI, owner);
        execute(rData);
    }
